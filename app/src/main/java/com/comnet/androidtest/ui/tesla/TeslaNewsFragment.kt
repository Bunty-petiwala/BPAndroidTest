package com.comnet.androidtest.ui.tesla
...
import com.comnet.androidtest.databinding.FragmentTeslaNewsBinding
...
class TeslaNewsFragment : Fragment() {
    ...
    private fun getNews(query: String) {
        mViewModel.getNewsArticles(query=query,
            from = DateUtils().getYesterdayDate(mCalendar)).observe(viewLifecycleOwner, androidx.lifecycle.Observer {
            when {
                it.status.isLoading() -> {
